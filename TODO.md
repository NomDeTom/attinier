...
- [x] Open P-MOSFET when supercap voltage within 95% of bus voltage (PB3, note: currently active-high, may need inversion for true P-MOSFET active-low behavior)
...
- [ ] Fast-charge to 4.5V supercap chemistry option
  - Define supercap chemistry profile with 4.5 V charge target
  - Add supercap chemistry selection mechanism (may reuse existing chemistry select ladder or add separate pin)
  - Integrate into BatteryProfiles and charging flow
...
- [ ] Test switch input (active low) that drives a complementary output pin high
  - Define test switch input pin (active low)
  - Define complementary output pin (driven high when switch is low)
  - Add debouncing logic if needed
  - Integrate into main loop or ISR as appropriate
